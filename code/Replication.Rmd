...
ggplot(fig3_df, aes(x = factor(year.dummy))) +
...
ggplot(fig4_df, aes(x = year.dummy, y = word_count, color = as.factor(cabinet))) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  labs(x = "Session",
...
ggplot(coef.frame, aes(x = sess, y = est)) +
  geom_point() +
...
fig9_df <- big.frame %>%
  group_by(year.dummy, cabinet) %>%
  dplyr::summarise(speech_count = n()) %>%
  group_by(year.dummy) %>%
  mutate(
    total_speeches = sum(speech_count),
    cabinet_prop = sum(ifelse(cabinet == 1, speech_count, 0)) / total_speeches,
    non_cabinet_prop = sum(ifelse(cabinet == 0, speech_count, 0)) / total_speeches
  ) %>%
  select(year.dummy, cabinet_prop, non_cabinet_prop) %>%
  distinct()  # keep only unique session values



fig9_df$year.dummy <- as.factor(fig9_df$year.dummy)

ggplot(fig9_df, aes(x = as.factor(year.dummy))) +
  geom_line(aes(y = cabinet_prop, color = "Cabinet")) +
  geom_line(aes(y = non_cabinet_prop, color = "Non-Cabinet")) +
  labs(title = "Proportion of Speeches Over Time",
       x = "Session",
       y = "Proportion of Speeches",
       color = "Member Type") +
  scale_x_discrete(breaks = unique(fig9_df$year.dummy)[seq(1, length(unique(fig9_df$year.dummy)), length.out = 10)]) +
  theme_minimal() +
  scale_color_manual(values = c("Cabinet" = "pink", "Non-Cabinet" = "green")) +
  theme(legend.position = c(0.10, 0.10)) +
  guides(color = guide_legend(override.aes = list(fill = c("pink", "green")),
                              title = "Legend",
                              keywidth = 1.5,
